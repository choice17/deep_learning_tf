...
class DNN:
    ...
    def conv2d(self,data,kernel ,strides=[1,1,1,1], padding='SAME', actu = 'relu',
        ...
        # conv here
        x = Layers.add('conv2d', data=x,opts=['weights',w,'strides',s,'padding',p],name=conv_name) + b

        # activation here
        x = Layers.add('relu', data=x,opts=[],name=a_name)

        # batchnorm here
        if batchnorm == True:
            x = Layers.add('batchnorm', data=x, name = bn_name)

        if pooling is not None:
            x = Layers.add(pooling, data=x, opts=['strides',s,'padding',p],name=pool_name)
...
class Layers:
    ...
    def add(self,layer_type=None,data=None,opts=None,name=None):
...
class layer:
    '''
    calling plain tensorflow library
    from tf.nn '''
